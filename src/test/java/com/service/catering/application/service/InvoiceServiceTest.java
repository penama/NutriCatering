...
public class InvoiceServiceTest {
  @Mock private IQueryInvoiceRepository iQueryInvoiceRepository;
  @Mock private IQueryInvoiceDetailRepository iQueryInvoiceDetailRepository;
  @Mock private IGenerateInvoiceRepository iGenerateInvoiceRepository;
  ...
  public void testGetInvoices() throws Exception {
    ...
    Mockito.when(
            iQueryInvoiceDetailRepository.queryInvoiceDetailByInvoice(Mockito.any(String.class)))
        .thenReturn(listInvoiceDetail);

    // Simulación de métodos estáticos con mockStatic()
    try (MockedStatic<InvoiceUtil> mockedStatic = mockStatic(InvoiceUtil.class)) {
      mockedStatic
          .when(() -> InvoiceUtil.InvoiceEntityToInvoiceDto(invoiceEntity))
          .thenReturn(invoiceDto);

      try (MockedStatic<InvoiceDetailUtil> mockedStaticDetail =
          mockStatic(InvoiceDetailUtil.class)) {
        mockedStaticDetail
            .when(() -> InvoiceDetailUtil.InvoiceDetailEntityToInvoiceDetail(invoiceDetailEntity))
  ...
  public void testGenerateInvoice() throws Exception {
    ...
    Mockito.doNothing()
        .when(iGenerateInvoiceRepository)
        .generateInvoice(Mockito.any(InvoiceEntity.class));
    Mockito.doNothing()
        .when(iGenerateInvoiceDetailRepository)
        .generateInvoiceDetail(Mockito.any(InvoiceDetailEntity.class));

    // Simulación de métodos estáticos con mockStatic()
    try (MockedStatic<InvoiceUtil> mockedStatic = mockStatic(InvoiceUtil.class)) {
      mockedStatic
          .when(() -> InvoiceUtil.InvoiceDtoToInvoiceEntity(invoiceDto))
          .thenReturn(invoiceEntity);

      try (MockedStatic<InvoiceDetailUtil> mockedStaticDetail =
          mockStatic(InvoiceDetailUtil.class)) {
        mockedStaticDetail
            .when(() -> InvoiceDetailUtil.InvoiceDetailToInvoiceDetailEntity(invoiceDetail))
