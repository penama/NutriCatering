...
//        contractEntityMock.totalAmount = 100;
        ...
        try (MockedStatic<ContractUtil> mockedStatic = mockStatic(ContractUtil.class)) {
            mockedStatic.when(() -> ContractUtil.contractDtoToContractEntity(contractDto)).thenReturn(contractEntityMock);
...
//            doThrow(new RuntimeException("Error al generar órdenes")).when(iOrderServiceCreateByContract).generateOrdersForContract(contractEntityMock.id);
//            // Llamar al método a probar
//            try {
//                contractService.newContract(contractDto);
//            } catch (Exception e) {
...
//        contractEntityMock.totalAmount = 100;
        ...
        // Simulación de métodos estáticos con mockStatic()
        try (MockedStatic<ContractUtil> mockedStatic = mockStatic(ContractUtil.class)) {
            mockedStatic.when(() -> ContractUtil.contractEntityToContractDto(contractEntityMock)).thenReturn(contractDto);
...
//            assertEquals(expectedDtoList, result); // Comparar listas
            ...
            mockedStatic.verify(() -> ContractUtil.contractEntityToContractDto(contractEntityMock), times(1));
...
//        contractEntityMock.totalAmount = 100;
        ...
        // Simulación del repositorio
        when(iQueryContractRepository.queryContractsCustomerId(Mockito.any(String.class))).thenReturn(expectedDtoList);
        // Simulación de métodos estáticos con mockStatic()
        try (MockedStatic<ContractUtil> mockedStatic = mockStatic(ContractUtil.class)) {
            mockedStatic.when(() -> ContractUtil.contractEntityToContractDto(contractEntityMock)).thenReturn(contractDto);
...
////        when(iQueryContractRepository.queryContractId(Mockito.any(String.class))).thenReturn( contractEntityNUll );
//        // Simulación de métodos estáticos con mockStatic()
//        try (MockedStatic<ContractUtil> mockedStatic = mockStatic(ContractUtil.class)) {
//            mockedStatic.when(() -> ContractUtil.contractEntityToContractDto(contractEntityMock)).thenReturn(contractDto);
//
...
//
//            // Simular que generateOrdersForContract() lanza una excepción
////            doThrow(new RuntimeException("Error al generar órdenes")).when(iQueryContractRepository).queryContractId( contractEntityMock.id );
//
